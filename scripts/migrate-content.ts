  ...
  for (let i = 0; i < locales.length; i++) {
    ...
    await payload.updateGlobal({
      ...
      data: {
        ...
        commonViewAllServices: t?.common?.viewAllServices ?? '',
        commonOrderCall: t?.common?.orderCall ?? '',
        commonGetConsultation: t?.common?.getConsultation ?? '',
        navTariffs: t?.header?.navigation?.tariffs ?? (locale === 'ru' ? 'Тарифы' : 'Tariffs'),
        navKnowledge: t?.header?.navigation?.knowledge ?? (locale === 'ru' ? 'База знаний' : 'Knowledge Base'),
        navAbout: t?.header?.navigation?.about ?? (locale === 'ru' ? 'О нас' : 'About'),
